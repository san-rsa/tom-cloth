...
import { ShoppingCart, Heart, BarChart2, Star, Check, HelpCircle, XIcon, Delete, DeleteIcon, HeartX } from "lucide-react";
...
function Detailimages({productData,}) {
  ...
  // {/* Gallery */}
    ...
    const [selectedImage, setSelectedImage] = useState( productData?.img[0]?.url);
                      ...
                      useEffect(() => {
                         setSelectedImage(productData?.img[0]?.url)
                      }, [productData]);
  ...
  // {/* Gallery */}
    ...
    const handleQuantityChange = (type) => {
      ...
      if (type === "inc") setQuantity(quantity + 1);
                         ...
                         setSelectedSize({size: productData.size[0].size, price: productData.size[0].price, _id: productData.size[0]._id})
                         setQuantity(1)
                         setwish(!wishlist)
                      }, [productData]);
                                                  ...
                                                  useEffect(() => {
                        fetch(process.env.REACT_APP_API_LINK  + "getone/wishlist/" + productData._id, {
                                  credentials: "include",
                              ...
                              .then((data) =>  {
                                  if (data === true) {
                                      setwish(true)                                
                                  console.log(data, wishlist, 2);

                                  } else if (data === false) {
                                      setwish(false)                                 
                                       console.log(data, wishlist , 6);

                                  } else if (String(data) === 'you have no wishlist')  {
                                      setwish(false)
                                  console.log(data, wishlist, 1);
                                  ...
                                  console.log(data, wishlist);
                                                                            ...
                                                                            useEffect(() => {
                        fetch(process.env.REACT_APP_API_LINK  + "getall/wishlists/" , {
                                  credentials: "include",
                                  headers: { "Content-type": "application/json; charset=UTF-8", },
                              }).then((res) =>  res.json())
                              .then((data) =>  {
                          ...
                          console.log(wishlist);
                               ...
                               function wish(e) {
                                  ...
                                  if (wishlist === false) {
                                   ...
                                   .then( ()=> setwish(true), AlertSuccess('successfully added to wishlist '))
                      
                      
                      
                                  } else if (wishlist === true) {
                                      fetch(process.env.REACT_APP_API_LINK + "delete/wishlist", {
                                          method: "DELETE",
                                          credentials: "include",
                                          headers: {
                                            "Content-type": "application/json",
                                       ...
                                       .then( ()=> setwish(false), AlertSuccess('successfully removed to wishlist '))
        ...
        <div className={styles.buyPanel}>
          ...
          <div className={styles.actionSuite}>
            ...
            <div className={styles.metaActionsRow}>
             {loggedin ?  <button onClick={wish} className={styles.metaBtn}>
               {wishlist ?  <span> <HeartX size={16} /> REMOVE TO WISH LIST </span>  : <span> <Heart size={16} /> ADD TO WISH LIST </span>   }
              </button> : null}
